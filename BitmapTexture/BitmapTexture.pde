...
void setup() {
  size(900, 900, P3D);
  smooth(8);
  img = loadImage("houseimage (625).png");
  img.resize(800, 800);
  ...
  for (int x=0; x<img.width; x++) {
    for (int y=0; y<img.height; y++) {
      ...
      float current = brightness(img.pixels[inc]);
      //if your image is black and white, use this conditional
      //if (current<1) {
      //  current=0;
      //} else {
      //  current=5;
      //}
      //c[inc] = int(current);
      //if your image is greyscale, use this mapping
      c[inc] = int(map(current, 0, 255, 0, 5));
...
void keyPressed() {
  ...
  if (key=='s') {
    saveFrame("saved/houseimage625_cam" + currentCam + "_" + hour() + second()+".png");
  }
}
